...
class TestBasicDataPreprocessor(unittest.TestCase):
    ...
    def test_process_string_normal_case_n3(self):
        ...
        np.testing.assert_array_almost_equal(obs, np.array([10.0, 21.0, -5.0], dtype=np.float32))

    def test_process_string_normal_case_n2(self):
        obs = self.preprocessor.process("10.2,20.7,-5.3", n=2)
        np.testing.assert_array_almost_equal(obs, np.array([10.0, 21.0], dtype=np.float32))

    def test_process_string_fewer_elements_than_n(self):
        obs = self.preprocessor.process("10.2", n=3)
        np.testing.assert_array_almost_equal(obs, np.array([10.0, 0.0, 0.0], dtype=np.float32))
    ...
    def test_process_string_non_numeric_first(self):
        ...
        np.testing.assert_array_almost_equal(obs, np.array([0.0, 0.0, 0.0], dtype=np.float32))
    ...
    def test_process_list_input_n_is_kwarg_only(self):
        # n is a kwarg, should not affect list processing length unless explicitly handled
        # Current BasicDataPreprocessor.process uses n for lists too (implicitly by values = [float(x) for x in sensor_data])
        # and then quantizes. The `[0:n]` slice was only for string parsing.
        # The new string parsing logic is `selected_parts = parts[:n]`.
        # For lists, it's `values = [float(x) for x in sensor_data]`. Then quantization.
        # So, n doesn't limit list length for parsing, only for string.
        obs = self.preprocessor.process([10.2, 20.7, -5.3, 99.9], n=2) # n=2 should be ignored for list input
        np.testing.assert_array_almost_equal(obs, np.array([10.0, 21.0, -5.0, 100.0], dtype=np.float32))
    ...
    def test_quantization_and_thresholding(self):
        ...
        obs = self.preprocessor_custom.process("0.3,0.7,0.05", n=3) # 0.05 should be zeroed
        np.testing.assert_array_almost_equal(obs, np.array([0.5, 0.5, 0.0], dtype=np.float32))

        obs_near_zero = self.preprocessor_custom.process("0.09,-0.09", n=2) # Should be zeroed by min_threshold
        np.testing.assert_array_almost_equal(obs_near_zero, np.array([0.0, 0.0], dtype=np.float32))
