...
# Assuming serial module would be available in the test environment,
# or we mock its exceptions if needed.
import serial # Keep this for serial.SerialException
...
class TestSerialCommunicator(unittest.TestCase):
    ...
    def test_connect_successful(self, mock_serial_class):
        ...
        communicator = SerialCommunicator(self.port, self.baud_rate, self.timeout, self.logger)
        self.assertTrue(communicator.connect())
        self.assertTrue(communicator.is_open())
        mock_serial_class.assert_called_once_with(self.port, self.baud_rate, timeout=self.timeout)
    ...
    def test_connect_persistent_failure(self, mock_sleep, mock_serial_class):
        mock_serial_class.side_effect = serial.SerialException("Connection failed persistently")

        communicator = SerialCommunicator(self.port, self.baud_rate, self.timeout, self.logger)
        with self.assertRaises(ConnectionError) as context:
            communicator.connect(retries=3, delay=0.1)

        self.assertIn("Failed to connect to /dev/ttyTest after 3 attempts", str(context.exception))
    ...
    def test_close_connection(self, mock_serial_class):
        ...
        # is_open should reflect the underlying mock's state if not explicitly managed
        # For simplicity, we assume ser.is_open would be false after close.
        # To test communicator.is_open() accurately, it should check self.ser.is_open.
    ...
    def test_read_line_serial_exception(self, mock_serial_class):
        mock_serial_instance = MagicMock()
        mock_serial_instance.is_open = True
        mock_serial_instance.readline.side_effect = serial.SerialException("Read failed")
        mock_serial_class.return_value = mock_serial_instance
    ...
    @patch('serial.Serial')
    @patch('morph_drive.robots.serial_communicator.time.time') # Mock time.time for timeout
    @patch('morph_drive.robots.serial_communicator.sleep') # Mock sleep
    def test_wait_for_ready_successful(self, mock_sleep, mock_time, mock_serial_class):
        mock_serial_instance = MagicMock()
        mock_serial_instance.is_open = True
        mock_serial_instance.readline.side_effect = [b"INIT\r\n", b"READY\r\n"]
        mock_serial_class.return_value = mock_serial_instance

        # Simulate time progression for timeout
        mock_time.side_effect = [100.0, 100.1, 100.2] # Start, after first read, after second read

        communicator = SerialCommunicator(self.port, self.baud_rate, self.timeout, self.logger)
        communicator.connect()

        self.assertTrue(communicator.wait_for_ready(ready_signal="READY", timeout_seconds=1.0))
        self.assertEqual(mock_serial_instance.readline.call_count, 2) # INIT then READY
        mock_serial_instance.reset_input_buffer.assert_any_call() # Called by flush_input

    @patch('serial.Serial')
    @patch('morph_drive.robots.serial_communicator.time.time') # Mock time.time for timeout
    @patch('morph_drive.robots.serial_communicator.sleep') # Mock sleep
    def test_wait_for_ready_timeout(self, mock_sleep, mock_time, mock_serial_class):
        mock_serial_instance = MagicMock()
        mock_serial_instance.is_open = True
        mock_serial_instance.readline.return_value = b"WAITING\r\n" # Never sends READY
        mock_serial_class.return_value = mock_serial_instance

        # Simulate time progression: start_time, then repeatedly past timeout
        time_sequence = [100.0] + [100.0 + 0.2 * i for i in range(10)]
        mock_time.side_effect = time_sequence

        communicator = SerialCommunicator(self.port, self.baud_rate, self.timeout, self.logger)
        communicator.connect()

        self.assertFalse(communicator.wait_for_ready(ready_signal="READY", timeout_seconds=0.5))
        # Check that sleep was called multiple times and readline was called multiple times
        self.assertTrue(mock_serial_instance.readline.call_count > 1)
    ...
    def test_context_manager(self, mock_serial_class):
        ...
        with SerialCommunicator(self.port, self.baud_rate, self.timeout, self.logger) as comm:
